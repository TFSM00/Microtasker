...
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ServerAliveInterval=100 "$SSH_USER@$SSH_HOST" -p "$SSH_PORT" "$remote_command" < /tmp/workspace.tar.bz2 